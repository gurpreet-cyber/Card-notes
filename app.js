...
function CarouselScreen({ stack, onBack, onUpdateStack }) {
  ...
  const exportAsJpeg = async (scope = "current") => {
    ...
    const renderCard = (note, idx, total, ctx, offsetX = 0, offsetY = 0) => {
      const W = 600, H = 600;

      // Compute the exact same card background colour as the live card
      ...
      // Compute text/meta colours adapted to the card background (same logic as adaptColours)
      const hexLum = (hex) => {
        const h = hex.replace('#','');
      ...
      const lum = cardBg.startsWith('#') ? hexLum(cardBg) : 0.5;
      const textColor = lum < 0.35 ? "#f0f0f0" : "#111111";
      const metaColor = lum < 0.35 ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.35)";

      // Canvas background = card colour — no white border
      ctx.fillStyle = cardBg;
      ctx.fillRect(offsetX, offsetY, W, H);

      // Page number (only on middle cards)
      if ((idx > 0 && idx < total - 1) || (total === 2 && idx === 1)) {
        ctx.fillStyle = metaColor;
        ctx.font = "500 22px Inter, sans-serif";
        ctx.textBaseline = "top";
        const numStr = `${idx + 1}`;
        const totStr = `${total}`;
        ctx.fillText(numStr, 40, 40);
        ctx.fillStyle = lum < 0.35 ? "rgba(255,255,255,0.25)" : "rgba(0,0,0,0.18)";
        ctx.fillText("/", 40 + ctx.measureText(numStr).width + 4, 40);
        ctx.fillStyle = metaColor;
        ctx.fillText(totStr, 40 + ctx.measureText(numStr + "/").width + 8, 40);
      }

      // Parse and render each line — font sizes match DEFAULT_TYPO × 2 (canvas is 2× card)
      const rawLines = (note.text || "").split("\n");
      const hFont = T.fonts && T.fonts.heading ? `'${T.fonts.heading}', serif` : "Inter, sans-serif";
      const bFont = T.fonts && T.fonts.body ? `'${T.fonts.body}', sans-serif` : "Inter, sans-serif";
      const LEFT = offsetX + 40;
      const MAX_W = W - 80;
      const TOP = offsetY + 88;
      const BOTTOM_LIMIT = offsetY + H - 48;

      // DEFAULT_TYPO values × 2, lineHeight = fontSize × 1.6 (matches app)
      const lineStyles = {
        h1: { fontSize: 44, fontWeight: 700, lineHeight: 60, letterSpacing: -0.8, fontFamily: hFont },
        h2: { fontSize: 34, fontWeight: 700, lineHeight: 48, letterSpacing: -0.4, fontFamily: hFont },
        h3: { fontSize: 26, fontWeight: 600, lineHeight: 38, letterSpacing: 1.0, upper: true, fontFamily: hFont },
        li: { fontSize: 28, fontWeight: 400, lineHeight: 45, bullet: true, fontFamily: bFont },
        ol: { fontSize: 28, fontWeight: 400, lineHeight: 45, fontFamily: bFont },
        p:  { fontSize: 28, fontWeight: 400, lineHeight: 45, fontFamily: bFont },
      };
      ...
      for (let li = 0; li < rawLines.length; li++) {
        ...
        for (let wi = 0; wi < words.length; wi++) {
          ...
          if (ctx.measureText(test).width > MAX_W && line) {
            if (curY + s.fontSize > BOTTOM_LIMIT) {
        ...
        if (line && curY + s.fontSize <= BOTTOM_LIMIT) {
          ctx.fillText(line, LEFT, curY);
          curY += s.lineHeight;
        }

        // Gap between lines
        if (li < rawLines.length - 1) curY += 4;
    ...
    if (scope === "current") {
      ...
      renderCard(note, active, notes.length, ctx);
      const a = document.createElement("a");
      a.href = canvas.toDataURL("image/jpeg", 0.95);
      a.download = `${name}-card-${active + 1}.jpg`; a.click();
    } else {
      // All cards — vertical strip, no padding/bg
      const filled = notes.filter(n => n.text.trim());
      if (!filled.length) return;
      const W = 600, H = 600, GAP = 8;
      const canvas = document.createElement("canvas");
      canvas.width = W;
      canvas.height = filled.length * H + (filled.length - 1) * GAP;
      const ctx = canvas.getContext("2d");
      filled.forEach((note, i) => {
        renderCard(note, notes.indexOf(note), notes.length, ctx, 0, i * (H + GAP));
      });
      const a = document.createElement("a");
      a.href = canvas.toDataURL("image/jpeg", 0.95);
      a.download = `${name}-all-cards.jpg`; a.click();
    }
  };
  ...
  return (
    React.createElement('div', { style: { width: "100%", maxWidth: 440, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 60, position: "relative" },
      onClick: () => { setShowDownloadMenu(false); setShowThemePicker(false); }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1672}}
      /* Back button */
      , React.createElement('div', { style: { position: "absolute", top: 16, left: 20 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1675}}
        ...
        }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1676}}
          , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1685}}
            , React.createElement('polyline', { points: "15 18 9 12 15 6"     , __self: this, __source: {fileName: _jsxFileName, lineNumber: 1686}})
          ), "All notes"
      ...
      , React.createElement('div', { style: { position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", zIndex: 100000 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1693}}
        , React.createElement('button', { onClick: () => { setShowThemePicker(v => !v); setShowDownloadMenu(false); },
          ...
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1705}}
            , React.createElement('div', { style: { height: 1.5, borderRadius: 1, background: theme.text, opacity: 0.8, width: "80%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1711}} )
            , React.createElement('div', { style: { height: 1, borderRadius: 1, background: theme.text, opacity: 0.35, width: "55%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1712}} )
          )
          , theme.label
        )
        ...
        , showThemePicker && (
          React.createElement('div', { style: {
            ...
            onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1718}}

            , THEME_GROUPS.map((group, gi) => (
              React.createElement('div', { key: group.group, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1728}}
                /* Group header */
                , React.createElement('div', { style: {
                  padding: gi === 0 ? "14px 16px 2px" : "12px 16px 2px",
                  display: "flex", flexDirection: "column", gap: 1,
                }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1730}}
                  , React.createElement('span', { style: { fontSize: 11, fontWeight: 700, color: "#111", fontFamily: "'Inter', sans-serif", letterSpacing: 0.3 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1734}}
                    , group.group.toUpperCase()
                  )
                  , group.desc && (
                    React.createElement('span', { style: { fontSize: 10.5, color: "#999", fontFamily: "'Inter', sans-serif", fontStyle: "italic" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1738}}
                      , group.desc
                    )
                  )
                )
                ...
                , React.createElement('div', { style: { padding: "4px 10px 6px", display: "flex", flexDirection: "column", gap: 2 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1745}}
                  , group.themes.map(t => {
                    ...
                    return (
                      React.createElement('button', {
                        ...
                        onMouseLeave: e => { if (!isActive) e.currentTarget.style.background = "none"; }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1749}}
                        ...
                        }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1764}}
                          , React.createElement('div', { style: { height: 2.5, borderRadius: 1.5, background: t.text, opacity: 0.85, width: "75%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1774}} )
                          , React.createElement('div', { style: { height: 1.5, borderRadius: 1, background: t.text, opacity: 0.35, width: "55%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1775}} )
                          , React.createElement('div', { style: { height: 1.5, borderRadius: 1, background: t.text, opacity: 0.35, width: "65%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1776}} )
                          /* Meta dot */
                          , React.createElement('div', { style: {
                            position: "absolute", bottom: 4, right: 5,
                            width: 4, height: 4, borderRadius: "50%",
                            background: t.meta, opacity: 0.7,
                          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1778}} )
                        )

                        /* Text info */
                        , React.createElement('div', { style: { flex: 1, minWidth: 0 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1786}}
                          , React.createElement('div', { style: {
                            fontSize: 12, fontWeight: isActive ? 600 : 500,
                            color: "#111", fontFamily: "'Inter', sans-serif",
                            lineHeight: 1.3,
                          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1787}}, t.label)
                          , React.createElement('div', { style: {
                            fontSize: 10.5, color: "#888",
                            fontFamily: "'Inter', sans-serif",
                            lineHeight: 1.4, marginTop: 1,
                            whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
                          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1792}}, t.desc)
                        )

                        /* Active checkmark */
                        , isActive && (
                          React.createElement('div', { style: { flexShrink: 0, color: "#111", fontSize: 13, fontWeight: 600 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1802}}, "✓")
                        )
                      )
                    );
                  })
                ...
                , gi < THEME_GROUPS.length - 1 && (
                  React.createElement('div', { style: { height: 1, background: "#f0f0f0", margin: "0 16px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1810}} )
                )
              )
            ))
          )
      ...
      , notes.length > 1 && (
        React.createElement('div', { style: { position: "absolute", top: 16, right: 62 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1820}}
          ...
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1821}}
            , React.createElement('svg', { width: "15", height: "15", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1827}}
              , React.createElement('line', { x1: "3", y1: "6", x2: "21", y2: "6", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1828}}), React.createElement('line', { x1: "3", y1: "12", x2: "21", y2: "12", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1828}}), React.createElement('line', { x1: "3", y1: "18", x2: "21", y2: "18", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1828}})
            )
          )
        )
      )
      ...
      /* Reorder button */
      , notes.length > 1 && (
        React.createElement('div', { style: { position: "absolute", top: 16, right: 62 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1836}}
          , React.createElement('button', { onClick: () => { setShowReorder(true); setShowThemePicker(false); setShowDownloadMenu(false); }, style: {
            width: 34, height: 34, borderRadius: "50%",
            border: "1px solid #e5e5e5", background: showReorder ? "#f0f0f0" : "#fff",
            cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
            boxShadow: "0 1px 4px rgba(0,0,0,0.06)", transition: "background .15s",
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1837}}
            , React.createElement('svg', { width: "15", height: "15", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1843}}
              , React.createElement('line', { x1: "3", y1: "6", x2: "21", y2: "6", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1844}}), React.createElement('line', { x1: "3", y1: "12", x2: "21", y2: "12", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1844}}), React.createElement('line', { x1: "3", y1: "18", x2: "21", y2: "18", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1844}})
            )
          )
        )
      )
      ...
      , React.createElement('div', { style: { position: "absolute", top: 16, right: 20 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1852}}
        , React.createElement('button', { onClick: () => setShowDownloadMenu(v => !v), style: {
          ...
          onMouseLeave: e => { e.currentTarget.style.boxShadow = "0 1px 4px rgba(0,0,0,0.06)"; e.currentTarget.style.borderColor = "#e5e5e5"; }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1853}}
          , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1864}}
            , React.createElement('path', { d: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"          , __self: this, __source: {fileName: _jsxFileName, lineNumber: 1865}})
            , React.createElement('polyline', { points: "7 10 12 15 17 10"     , __self: this, __source: {fileName: _jsxFileName, lineNumber: 1866}})
            , React.createElement('line', { x1: "12", y1: "15", x2: "12", y2: "3", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1867}})
          )
        )
        ...
        , showDownloadMenu && (
          ...
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1873}}
            /* Image section */
            , React.createElement('div', { style: { padding: "8px 16px 4px", fontSize: 10, fontWeight: 600, color: "#aaa", fontFamily: "'Inter', sans-serif", letterSpacing: 1, textTransform: "uppercase" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1880}}, "Image")
            , [
              ...
              React.createElement('button', { key: scope, onClick: () => exportAsJpeg(scope), style: {
                ...
                onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1885}}
                , React.createElement('span', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1894}}, label)
                , React.createElement('span', { style: { fontSize: 11, color: "#aaa" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1895}}, ".jpg")
              )
            ))

            /* Divider */
            , React.createElement('div', { style: { height: 1, background: "#f0f0f0", margin: "4px 0" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1900}} )

            /* Text section */
            , React.createElement('div', { style: { padding: "8px 16px 4px", fontSize: 10, fontWeight: 600, color: "#aaa", fontFamily: "'Inter', sans-serif", letterSpacing: 1, textTransform: "uppercase" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1903}}, "Text")
            , [
              ...
              React.createElement('button', { key: fmt, onClick: () => handleDownload(fmt), style: {
                ...
                onMouseEnter: e => e.currentTarget.style.background = "#f7f7f5",
                onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1909}}
                , React.createElement('span', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1918}}, label)
                , React.createElement('span', { style: { fontSize: 11, color: "#aaa" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1919}}, ext)
              )
            ))
            , React.createElement('div', { style: { height: 8 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1922}} )
          )
        )
      )
      ...
      , React.createElement('div', {
        ...
        onPointerCancel: onPointerUp, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1928}}
        ...
        , visibleNotes.map((note, i) => {
          ...
          return (
            React.createElement('div', { key: `${note.id}-${theme && theme.id}`,
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1948}}
              , React.createElement(NoteCard, { note: note, isActive: isAct && !note.isGhost,
                ...
                theme: theme, cardIndex: note.index, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1962}} )
            )
          );
        })
      )
      ...
      , React.createElement('div', { style: { height: 40 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1973}} )

      /* ── Reorder sheet ── */
      , showReorder && (
        React.createElement('div', {
          style: { position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.4)" },
          onClick: () => setShowReorder(false), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1977}}
          ...
          , React.createElement('div', {
            ...
            onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1981}}

            /* Handle */
            , React.createElement('div', { style: { display: "flex", justifyContent: "center", paddingTop: 12 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1991}}
              , React.createElement('div', { style: { width: 36, height: 4, borderRadius: 2, background: "#e0e0e0" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1992}} )
            )

            /* Header */
            , React.createElement('div', { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px 8px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1996}}
              , React.createElement('span', { style: { fontFamily: "'Inter',sans-serif", fontSize: 15, fontWeight: 600, color: "#111" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1997}}, "Reorder Cards" )
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1998}}, "Done")
            )
            ...
            , React.createElement('div', {
              ...
              onPointerCancel: onRowPointerUp, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2005}}
              ...
              , notes.map((note, i) => {
                ...
                return (
                  React.createElement('div', { key: note.id, style: {
                    ...
                    position: 'relative',
                    marginBottom: 8,
                  }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2031}}
                    , React.createElement('div', {
                      ...
                      }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2053}}
                        , React.createElement('span', { style: { fontFamily: "'Inter',sans-serif", fontSize: 11, fontWeight: 700, color: theme.text, opacity: 0.5 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2058}}, i+1)
                      )
                      ...
                      }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2062}}, preview)

                      /* Grip */
                      , React.createElement('svg', { width: "18", height: "18", viewBox: "0 0 24 24"   , fill: "none", stroke: "#ccc", strokeWidth: "2", strokeLinecap: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2068}}
                        , React.createElement('circle', { cx: "8",  cy: "6",  r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2069}})
                        , React.createElement('circle', { cx: "16", cy: "6",  r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2070}})
                        , React.createElement('circle', { cx: "8",  cy: "12", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2071}})
                        , React.createElement('circle', { cx: "16", cy: "12", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2072}})
                        , React.createElement('circle', { cx: "8",  cy: "18", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2073}})
                        , React.createElement('circle', { cx: "16", cy: "18", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2074}})
                      )
                    )
                  )
                );
...
function HomeScreen({ stacks, onOpen, onNew, onDelete, onRename }) {
  ...
  return (
    React.createElement('div', { style: { width: "100%", maxWidth: 440, minHeight: "100vh", padding: "0 20px 40px", boxSizing: "border-box" },
      onClick: closeMenu, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2121}}
      ...
      , renaming && (
        ...
        }, onClick: e => { e.stopPropagation(); setRenaming(null); }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2126}}
          , React.createElement('div', { style: {
            background: "#fff", borderRadius: 16, padding: "24px 24px 20px",
            width: 280, boxShadow: "0 8px 40px rgba(0,0,0,0.18)",
          }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 2130}}
            , React.createElement('p', { style: { fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 600, color: "#000", marginBottom: 14 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2134}}, "Rename note" )
            , React.createElement('input', {
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2135}}
            )
            , React.createElement('div', { style: { display: "flex", gap: 8, marginTop: 16, justifyContent: "flex-end" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2148}}
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2149}}, "Cancel")
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2154}}, "Save")
            )
          )
        )
      )
      ...
      , menu && (
        React.createElement('div', { style: {
        ...
        }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 2166}}
          , React.createElement('button', { onClick: () => { handleRenameStart(menu.id); }, style: {
            ...
            onMouseEnter: e => e.currentTarget.style.background = "#f7f7f5",
            onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2174}}
            , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2183}}
              , React.createElement('path', { d: "M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"                 , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2184}})
              , React.createElement('path', { d: "M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"           , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2185}})
            ), "Rename"

          )
          , React.createElement('div', { style: { height: 1, background: "#f0f0f0", margin: "0 12px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2189}} )
          , React.createElement('button', { onClick: () => { onDelete(menu.id); closeMenu(); }, style: {
            ...
            onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2190}}
            , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2199}}
              , React.createElement('polyline', { points: "3 6 5 6 21 6"     , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2200}}), React.createElement('path', { d: "M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"            , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2200}})
              , React.createElement('path', { d: "M10 11v6M14 11v6"  , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2201}}), React.createElement('path', { d: "M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"            , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2201}})
            ), "Delete"
      ...
      /* Header */
      , React.createElement('div', { style: { padding: "52px 0 28px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2209}}
        , React.createElement('h1', { style: { fontFamily: "'Inter', sans-serif", fontSize: 24, fontWeight: 700, color: "#000", letterSpacing: -0.5 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2210}}, "Card Notes"
      ...
      /* Grid */
      , stacks.length === 0 ? (
        React.createElement('div', { style: { textAlign: "center", paddingTop: 80 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2217}}
          , React.createElement('p', { style: { fontFamily: "'Inter', sans-serif", fontSize: 14, color: "#bbb" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2218}}, "No notes yet. Tap + to create one."       )
        )
      ) : (
        React.createElement('div', { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2221}}
          , stacks.map(stack => (
            React.createElement('div', { key: stack.id, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2223}}
              , React.createElement('div', {
                ...
                style: { userSelect: "none", WebkitUserSelect: "none" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2224}}

                , React.createElement(StackCard, { stack: stack, theme: THEMES.find(t => t.id === (stack.themeId || 'white')) || THEMES[0], __self: this, __source: {fileName: _jsxFileName, lineNumber: 2234}} )
              )
              , React.createElement('p', { style: { fontFamily: "'Inter', sans-serif", fontSize: 11, color: "#aaa", marginTop: 6, paddingLeft: 2 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2236}}
                , stack.name && React.createElement('span', { style: { color: "#555", fontWeight: 500 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2237}}, stack.name, " · "  ), "Edited "
      ...
      , React.createElement('button', {
        ...
        onMouseLeave: e => { e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.25), 0 2px 8px rgba(0,0,0,0.15)"; }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2245}}

        , React.createElement('svg', { width: "22", height: "22", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2260}}
          , React.createElement('line', { x1: "12", y1: "5", x2: "12", y2: "19", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2261}}), React.createElement('line', { x1: "5", y1: "12", x2: "19", y2: "12", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2261}})
        )
      )
    )
  );
...
function App() {
  ...
  return (
    React.createElement(React.Fragment, null
      , React.createElement('style', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 2299}}, `
      ...
      }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2309}}
        , openId && activeStack ? (
          React.createElement(CarouselScreen, {
            key: openId,
            stack: activeStack,
            onBack: handleBack,
            onUpdateStack: handleUpdateStack, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2315}}
          )
        ) : (
          React.createElement(HomeScreen, { stacks: stacks, onOpen: handleOpen, onNew: handleNew, onDelete: handleDelete, onRename: handleRename, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2322}} )
        )
      )
    )
  );

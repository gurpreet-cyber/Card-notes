...
function NoteCard({ note, onChange, onDelete, isActive, count, active, onGoTo, theme, cardIndex, colorSeed }) {
  ...
  return (
    React.createElement('div', { 'data-card-active': isActive ? "true" : "false", style: {
      ...
      , React.createElement('div', { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 464}}
        , React.createElement(RadialCounter, { used: note.text.length, max: MAX_CHARS, metaColor: cardMeta, __self: this, __source: {fileName: _jsxFileName, lineNumber: 465}} )
        , React.createElement('div', { style: { display: "flex", gap: 4, alignItems: "center" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 466}}
          , (() => {
...
function CarouselScreen({ stack, onBack, onUpdateStack }) {
  ...
  const exportAsJpeg = async (scope = "current") => {
    setShowDownloadMenu(false);
    const name = stack.name || `note`;

    // Helper: render one note onto a canvas context at given offset
    const renderCard = (note, noteIdx, total, ctx) => {
      const W = 600, H = 600;
      const SCALE = 2; // canvas is 2× the on-screen card size

      // ── Exact same colour logic as NoteCard ──
      const T = theme || { card: "#ffffff", text: "#111111", meta: "#aaaaaa", border: "rgba(0,0,0,0.07)", palette: [] };
      const palette = T.palette && T.palette.length > 0 ? T.palette : [T.card];
      ...
      const lum = cardBg && cardBg.startsWith('#') ? hexLum(cardBg) : 0.5;
      const textColor = lum < 0.35 ? (T.text && hexLum(T.text||'#fff')>0.5 ? T.text : "#f0f0f0") : (T.text && hexLum(T.text||'#000')<0.3 ? T.text : "#111111");
      const metaColor = lum < 0.35 ? "rgba(255,255,255,0.45)" : "rgba(0,0,0,0.32)";

      // ── Full-bleed card background — no padding, no white border ──
      ctx.fillStyle = cardBg || "#ffffff";
      ctx.fillRect(0, 0, W, H);

      // ── Page number (same visibility logic as live card) ──
      if ((noteIdx > 0 && noteIdx < total - 1) || (total === 2 && noteIdx === 1)) {
        ctx.textBaseline = "top";
        ctx.font = `500 ${22}px Inter, -apple-system, sans-serif`;
        ctx.fillStyle = metaColor;
        const numStr = `${noteIdx + 1}`;
        const totStr = `${total}`;
        const slashX = 40 + ctx.measureText(numStr).width + 5;
        const totX = slashX + ctx.measureText("/").width + 5;
        ctx.fillText(numStr, 40, 36);
        ctx.fillStyle = lum < 0.35 ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.15)";
        ctx.fillText("/", slashX, 36);
        ctx.fillStyle = metaColor;
        ctx.fillText(totStr, totX, 36);
      }

      // ── Typography: use theme's actual typo values × SCALE ──
      const baseTypo = T.typo || {};
      const DEFAULT_T = { h1:{fontSize:22,fontWeight:700,letterSpacing:-0.4}, h2:{fontSize:17,fontWeight:700,letterSpacing:-0.2}, h3:{fontSize:13,fontWeight:600,letterSpacing:0.5,textTransform:"uppercase"}, li:{fontSize:14,fontWeight:400,letterSpacing:0}, ol:{fontSize:14,fontWeight:400,letterSpacing:0}, p:{fontSize:14,fontWeight:400,letterSpacing:0} };
      const hFont = T.fonts && T.fonts.heading ? `'${T.fonts.heading}', serif` : "Inter, -apple-system, sans-serif";
      const bFont = T.fonts && T.fonts.body ? `'${T.fonts.body}', sans-serif` : "Inter, -apple-system, sans-serif";

      const getStyle = (type) => {
        const base = { ...DEFAULT_T[type]||DEFAULT_T.p, ...(baseTypo[type]||{}) };
        const isH = type==="h1"||type==="h2"||type==="h3";
        return {
          fontSize:   Math.round((base.fontSize || 14) * SCALE),
          fontWeight: base.fontWeight || 400,
          lineHeight: Math.round((base.fontSize || 14) * SCALE * 1.55),
          letterSpacing: (base.letterSpacing || 0) * SCALE,
          textTransform: base.textTransform || "none",
          fontStyle:  base.fontStyle || "normal",
          fontFamily: isH ? hFont : bFont,
          bullet: type === "li",
          upper: (base.textTransform === "uppercase"),
        };
      };
      ...
      for (let li = 0; li < rawLines.length; li++) {
        ...
        let type = "p", content = raw;
        if (raw.startsWith("### "))     { type = "h3"; content = raw.slice(4); }
        else if (raw.startsWith("## ")) { type = "h2"; content = raw.slice(3); }
        else if (raw.startsWith("# "))  { type = "h1"; content = raw.slice(2); }
        else if (raw.startsWith("* "))  { type = "li"; content = raw.slice(2); }
        else { const m = raw.match(/^(\d+)\. /); if (m) { type = "ol"; content = raw.slice(m[0].length); olNumber = parseInt(m[1],10); } }

        const s = getStyle(type);
        const displayText = s.upper ? content.toUpperCase() : content;
        const bulletPrefix = s.bullet ? "• " : (type === "ol" ? `${olNumber}. ` : "");
        const fullText = bulletPrefix + displayText;

        ctx.font = `${s.fontStyle !== "normal" ? s.fontStyle + " " : ""}${s.fontWeight} ${s.fontSize}px ${s.fontFamily}`;
        ...
        for (let wi = 0; wi < words.length; wi++) {
          ...
          if (ctx.measureText(test).width > MAX_W && line) {
            if (curY + s.fontSize > BOTTOM_LIMIT) { ctx.fillText(line + "…", LEFT, curY); curY = BOTTOM_LIMIT + 1; break; }
            ctx.fillText(line, LEFT, curY);
            curY += s.lineHeight;
            line = words[wi];
          } else { line = test; }
        }
        if (line && curY + s.fontSize <= BOTTOM_LIMIT) { ctx.fillText(line, LEFT, curY); curY += s.lineHeight; }
        if (li < rawLines.length - 1) curY += 2;
        if (curY >= BOTTOM_LIMIT) break;
        if (type === "ol") olNumber++;
      }
    };
    ...
    const renderSingle = (note, noteIdx) => {
      const canvas = document.createElement("canvas");
      canvas.width = 600; canvas.height = 600;
      const ctx = canvas.getContext("2d");
      renderCard(note, noteIdx, notes.length, ctx);
      return canvas;
    };
    ...
    if (scope === "current") {
      ...
      if (!note) return;
      downloadCanvas(renderSingle(note, active), `${name}-card-${active + 1}.jpg`);
    } else {
      // Each card gets its own file — download with small delay between each
      const filled = notes.map((n, i) => ({ note: n, idx: i })).filter(({ note }) => note.text.trim());
      if (!filled.length) return;
      filled.forEach(({ note, idx }, i) => {
        setTimeout(() => {
          downloadCanvas(renderSingle(note, idx), `${name}-card-${idx + 1}.jpg`);
        }, i * 200); // 200ms stagger so browser doesn't block multiple downloads
      });
    }
  };
  ...
  return (
    React.createElement('div', { style: { width: "100%", maxWidth: 440, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 60, position: "relative" },
      onClick: () => { setShowDownloadMenu(false); setShowThemePicker(false); }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1679}}
      /* Back button */
      , React.createElement('div', { style: { position: "absolute", top: 16, left: 20 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1682}}
        ...
        }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1683}}
          , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1692}}
            , React.createElement('polyline', { points: "15 18 9 12 15 6"     , __self: this, __source: {fileName: _jsxFileName, lineNumber: 1693}})
          ), "All notes"
      ...
      , React.createElement('div', { style: { position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", zIndex: 100000 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1700}}
        , React.createElement('button', { onClick: () => { setShowThemePicker(v => !v); setShowDownloadMenu(false); },
          ...
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1701}}
          ...
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1712}}
            , React.createElement('div', { style: { height: 1.5, borderRadius: 1, background: theme.text, opacity: 0.8, width: "80%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1718}} )
            , React.createElement('div', { style: { height: 1, borderRadius: 1, background: theme.text, opacity: 0.35, width: "55%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1719}} )
          )
          , theme.label
        )
        ...
        , showThemePicker && (
          React.createElement('div', { style: {
            ...
            onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1725}}

            , THEME_GROUPS.map((group, gi) => (
              React.createElement('div', { key: group.group, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1735}}
                /* Group header */
                , React.createElement('div', { style: {
                  padding: gi === 0 ? "14px 16px 2px" : "12px 16px 2px",
                  display: "flex", flexDirection: "column", gap: 1,
                }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1737}}
                  , React.createElement('span', { style: { fontSize: 11, fontWeight: 700, color: "#111", fontFamily: "'Inter', sans-serif", letterSpacing: 0.3 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1741}}
                    , group.group.toUpperCase()
                  )
                  , group.desc && (
                    React.createElement('span', { style: { fontSize: 10.5, color: "#999", fontFamily: "'Inter', sans-serif", fontStyle: "italic" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1745}}
                      , group.desc
                    )
                  )
                )
                ...
                , React.createElement('div', { style: { padding: "4px 10px 6px", display: "flex", flexDirection: "column", gap: 2 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1752}}
                  , group.themes.map(t => {
                    ...
                    return (
                      React.createElement('button', {
                        ...
                        onMouseLeave: e => { if (!isActive) e.currentTarget.style.background = "none"; }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1756}}
                        ...
                        }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1771}}
                          , React.createElement('div', { style: { height: 2.5, borderRadius: 1.5, background: t.text, opacity: 0.85, width: "75%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1781}} )
                          , React.createElement('div', { style: { height: 1.5, borderRadius: 1, background: t.text, opacity: 0.35, width: "55%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1782}} )
                          , React.createElement('div', { style: { height: 1.5, borderRadius: 1, background: t.text, opacity: 0.35, width: "65%" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1783}} )
                          /* Meta dot */
                          , React.createElement('div', { style: {
                            position: "absolute", bottom: 4, right: 5,
                            width: 4, height: 4, borderRadius: "50%",
                            background: t.meta, opacity: 0.7,
                          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1785}} )
                        )

                        /* Text info */
                        , React.createElement('div', { style: { flex: 1, minWidth: 0 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1793}}
                          , React.createElement('div', { style: {
                            fontSize: 12, fontWeight: isActive ? 600 : 500,
                            color: "#111", fontFamily: "'Inter', sans-serif",
                            lineHeight: 1.3,
                          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1794}}, t.label)
                          , React.createElement('div', { style: {
                            fontSize: 10.5, color: "#888",
                            fontFamily: "'Inter', sans-serif",
                            lineHeight: 1.4, marginTop: 1,
                            whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
                          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1799}}, t.desc)
                        )

                        /* Active checkmark */
                        , isActive && (
                          React.createElement('div', { style: { flexShrink: 0, color: "#111", fontSize: 13, fontWeight: 600 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1809}}, "✓")
                        )
                      )
                    );
                  })
                ...
                , gi < THEME_GROUPS.length - 1 && (
                  React.createElement('div', { style: { height: 1, background: "#f0f0f0", margin: "0 16px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1817}} )
                )
              )
            ))
          )
      ...
      , notes.length > 1 && (
        React.createElement('div', { style: { position: "absolute", top: 16, right: 62 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1827}}
          ...
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1828}}
            , React.createElement('svg', { width: "15", height: "15", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1834}}
              , React.createElement('line', { x1: "3", y1: "6", x2: "21", y2: "6", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1835}}), React.createElement('line', { x1: "3", y1: "12", x2: "21", y2: "12", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1835}}), React.createElement('line', { x1: "3", y1: "18", x2: "21", y2: "18", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1835}})
            )
          )
        )
      )
      ...
      /* Reorder button */
      , notes.length > 1 && (
        React.createElement('div', { style: { position: "absolute", top: 16, right: 62 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1843}}
          , React.createElement('button', { onClick: () => { setShowReorder(true); setShowThemePicker(false); setShowDownloadMenu(false); }, style: {
            width: 34, height: 34, borderRadius: "50%",
            border: "1px solid #e5e5e5", background: showReorder ? "#f0f0f0" : "#fff",
            cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
            boxShadow: "0 1px 4px rgba(0,0,0,0.06)", transition: "background .15s",
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1844}}
            , React.createElement('svg', { width: "15", height: "15", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1850}}
              , React.createElement('line', { x1: "3", y1: "6", x2: "21", y2: "6", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1851}}), React.createElement('line', { x1: "3", y1: "12", x2: "21", y2: "12", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1851}}), React.createElement('line', { x1: "3", y1: "18", x2: "21", y2: "18", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1851}})
            )
          )
        )
      )
      ...
      , React.createElement('div', { style: { position: "absolute", top: 16, right: 20 }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1859}}
        , React.createElement('button', { onClick: () => setShowDownloadMenu(v => !v), style: {
          ...
          onMouseLeave: e => { e.currentTarget.style.boxShadow = "0 1px 4px rgba(0,0,0,0.06)"; e.currentTarget.style.borderColor = "#e5e5e5"; }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1860}}
          , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1871}}
            , React.createElement('path', { d: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"          , __self: this, __source: {fileName: _jsxFileName, lineNumber: 1872}})
            , React.createElement('polyline', { points: "7 10 12 15 17 10"     , __self: this, __source: {fileName: _jsxFileName, lineNumber: 1873}})
            , React.createElement('line', { x1: "12", y1: "15", x2: "12", y2: "3", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1874}})
          )
        )
        ...
        , showDownloadMenu && (
          ...
          }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1880}}
            /* Image section */
            , React.createElement('div', { style: { padding: "8px 16px 4px", fontSize: 10, fontWeight: 600, color: "#aaa", fontFamily: "'Inter', sans-serif", letterSpacing: 1, textTransform: "uppercase" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1887}}, "Image")
            , [
              ...
              React.createElement('button', { key: scope, onClick: () => exportAsJpeg(scope), style: {
                ...
                onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1892}}
                , React.createElement('span', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1901}}, label)
                , React.createElement('span', { style: { fontSize: 11, color: "#aaa" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1902}}, ".jpg")
              )
            ))

            /* Divider */
            , React.createElement('div', { style: { height: 1, background: "#f0f0f0", margin: "4px 0" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1907}} )

            /* Text section */
            , React.createElement('div', { style: { padding: "8px 16px 4px", fontSize: 10, fontWeight: 600, color: "#aaa", fontFamily: "'Inter', sans-serif", letterSpacing: 1, textTransform: "uppercase" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1910}}, "Text")
            , [
              ...
              React.createElement('button', { key: fmt, onClick: () => handleDownload(fmt), style: {
                ...
                onMouseEnter: e => e.currentTarget.style.background = "#f7f7f5",
                onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1916}}
                , React.createElement('span', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1925}}, label)
                , React.createElement('span', { style: { fontSize: 11, color: "#aaa" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1926}}, ext)
              )
            ))
            , React.createElement('div', { style: { height: 8 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1929}} )
          )
        )
      )
      ...
      , React.createElement('div', {
        ...
        onPointerCancel: onPointerUp, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1935}}
        ...
        , visibleNotes.map((note, i) => {
          ...
          return (
            React.createElement('div', { key: `${note.id}-${theme && theme.id}`,
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1955}}
              , React.createElement(NoteCard, { note: note, isActive: isAct && !note.isGhost,
                ...
                theme: theme, cardIndex: note.index, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1969}} )
            )
          );
        })
      )
      ...
      , React.createElement('div', { style: { height: 40 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1980}} )

      /* ── Reorder sheet ── */
      , showReorder && (
        React.createElement('div', {
          style: { position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.4)" },
          onClick: () => setShowReorder(false), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1984}}
          ...
          , React.createElement('div', {
            ...
            onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 1988}}

            /* Handle */
            , React.createElement('div', { style: { display: "flex", justifyContent: "center", paddingTop: 12 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1998}}
              , React.createElement('div', { style: { width: 36, height: 4, borderRadius: 2, background: "#e0e0e0" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1999}} )
            )

            /* Header */
            , React.createElement('div', { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px 8px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2003}}
              , React.createElement('span', { style: { fontFamily: "'Inter',sans-serif", fontSize: 15, fontWeight: 600, color: "#111" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2004}}, "Reorder Cards" )
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2005}}, "Done")
            )
            ...
            , React.createElement('div', {
              ...
              onPointerCancel: onRowPointerUp, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2012}}
              ...
              , notes.map((note, i) => {
                ...
                return (
                  React.createElement('div', { key: note.id, style: {
                    ...
                    position: 'relative',
                    marginBottom: 8,
                  }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2038}}
                    , React.createElement('div', {
                      ...
                      }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2045}}
                      ...
                      }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2060}}
                        , React.createElement('span', { style: { fontFamily: "'Inter',sans-serif", fontSize: 11, fontWeight: 700, color: theme.text, opacity: 0.5 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2065}}, i+1)
                      )
                      ...
                      }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2069}}, preview)

                      /* Grip */
                      , React.createElement('svg', { width: "18", height: "18", viewBox: "0 0 24 24"   , fill: "none", stroke: "#ccc", strokeWidth: "2", strokeLinecap: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2075}}
                        , React.createElement('circle', { cx: "8",  cy: "6",  r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2076}})
                        , React.createElement('circle', { cx: "16", cy: "6",  r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2077}})
                        , React.createElement('circle', { cx: "8",  cy: "12", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2078}})
                        , React.createElement('circle', { cx: "16", cy: "12", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2079}})
                        , React.createElement('circle', { cx: "8",  cy: "18", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2080}})
                        , React.createElement('circle', { cx: "16", cy: "18", r: "1.2", fill: "#ccc", stroke: "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2081}})
                      )
                    )
                  )
                );
...
function HomeScreen({ stacks, onOpen, onNew, onDelete, onRename }) {
  ...
  return (
    React.createElement('div', { style: { width: "100%", maxWidth: 440, minHeight: "100vh", padding: "0 20px 40px", boxSizing: "border-box" },
      onClick: closeMenu, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2128}}
      ...
      , renaming && (
        ...
        }, onClick: e => { e.stopPropagation(); setRenaming(null); }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2133}}
          , React.createElement('div', { style: {
            background: "#fff", borderRadius: 16, padding: "24px 24px 20px",
            width: 280, boxShadow: "0 8px 40px rgba(0,0,0,0.18)",
          }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 2137}}
            , React.createElement('p', { style: { fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 600, color: "#000", marginBottom: 14 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2141}}, "Rename note" )
            ...
            , React.createElement('div', { style: { display: "flex", gap: 8, marginTop: 16, justifyContent: "flex-end" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2155}}
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2156}}, "Cancel")
              ...
              }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2161}}, "Save")
            )
          )
        )
      )
      ...
      , menu && (
        React.createElement('div', { style: {
        ...
        }, onClick: e => e.stopPropagation(), __self: this, __source: {fileName: _jsxFileName, lineNumber: 2173}}
          , React.createElement('button', { onClick: () => { handleRenameStart(menu.id); }, style: {
            ...
            onMouseEnter: e => e.currentTarget.style.background = "#f7f7f5",
            onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2181}}
            , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2190}}
              , React.createElement('path', { d: "M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"                 , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2191}})
              , React.createElement('path', { d: "M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"           , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2192}})
            ), "Rename"

          )
          , React.createElement('div', { style: { height: 1, background: "#f0f0f0", margin: "0 12px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2196}} )
          , React.createElement('button', { onClick: () => { onDelete(menu.id); closeMenu(); }, style: {
            ...
            onMouseLeave: e => e.currentTarget.style.background = "none", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2197}}
            , React.createElement('svg', { width: "14", height: "14", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2206}}
              , React.createElement('polyline', { points: "3 6 5 6 21 6"     , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2207}}), React.createElement('path', { d: "M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"            , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2207}})
              , React.createElement('path', { d: "M10 11v6M14 11v6"  , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2208}}), React.createElement('path', { d: "M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"            , __self: this, __source: {fileName: _jsxFileName, lineNumber: 2208}})
            ), "Delete"
      ...
      /* Header */
      , React.createElement('div', { style: { padding: "52px 0 28px" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2216}}
        , React.createElement('h1', { style: { fontFamily: "'Inter', sans-serif", fontSize: 24, fontWeight: 700, color: "#000", letterSpacing: -0.5 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2217}}, "Card Notes"
      ...
      /* Grid */
      , stacks.length === 0 ? (
        React.createElement('div', { style: { textAlign: "center", paddingTop: 80 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2224}}
          , React.createElement('p', { style: { fontFamily: "'Inter', sans-serif", fontSize: 14, color: "#bbb" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2225}}, "No notes yet. Tap + to create one."       )
        )
      ) : (
        React.createElement('div', { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2228}}
          , stacks.map(stack => (
            React.createElement('div', { key: stack.id, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2230}}
              , React.createElement('div', {
                ...
                style: { userSelect: "none", WebkitUserSelect: "none" }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2231}}

                , React.createElement(StackCard, { stack: stack, theme: THEMES.find(t => t.id === (stack.themeId || 'white')) || THEMES[0], __self: this, __source: {fileName: _jsxFileName, lineNumber: 2241}} )
              )
              , React.createElement('p', { style: { fontFamily: "'Inter', sans-serif", fontSize: 11, color: "#aaa", marginTop: 6, paddingLeft: 2 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2243}}
                , stack.name && React.createElement('span', { style: { color: "#555", fontWeight: 500 }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2244}}, stack.name, " · "  ), "Edited "
      ...
      , React.createElement('button', {
        ...
        onMouseLeave: e => { e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.25), 0 2px 8px rgba(0,0,0,0.15)"; }, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2252}}

        , React.createElement('svg', { width: "22", height: "22", viewBox: "0 0 24 24"   , fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2267}}
          , React.createElement('line', { x1: "12", y1: "5", x2: "12", y2: "19", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2268}}), React.createElement('line', { x1: "5", y1: "12", x2: "19", y2: "12", __self: this, __source: {fileName: _jsxFileName, lineNumber: 2268}})
        )
      )
    )
  );
...
function App() {
  ...
  return (
    React.createElement(React.Fragment, null
      , React.createElement('style', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 2306}}, `
      ...
      , React.createElement('div', { style: {
        ...
        , openId && activeStack ? (
          React.createElement(CarouselScreen, {
            ...
            stack: activeStack,
            onBack: handleBack,
            onUpdateStack: handleUpdateStack, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2322}}
          )
        ) : (
          React.createElement(HomeScreen, { stacks: stacks, onOpen: handleOpen, onNew: handleNew, onDelete: handleDelete, onRename: handleRename, __self: this, __source: {fileName: _jsxFileName, lineNumber: 2329}} )
        )
      )
    )
  );
